...
PURE_TESTS = \
	...
	test_cli_status test_cli_tool_panel test_cli_tools test_cli_utf8 \
	test_cli_utf8_stream \
...
abi-check: $(LIB_SO_REAL)
	...
	status=0; \
	LC_ALL=C sort -u test/abi_exports.txt | diff -u - $$exports || status=$$?; \
	rm -f $$exports; \
	exit $$status
